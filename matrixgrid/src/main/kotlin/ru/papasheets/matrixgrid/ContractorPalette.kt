...
/**
 * Единый источник цветов подрядчиков (`colorIndex` → цвет) для всего приложения: и списка записей в
 * app, и матрицы. 10 базовых оттенков; тёмная ветка слегка приглушена, чтобы сплошные блоки LOD2 и
 * тонировка ячеек читались на тёмном фоне [MatrixColors]. Индекс заворачивается по модулю — подрядчик
 * за пределами палитры переиспользует цвет (палитра — акцент, а не идентификатор).
 */
object ContractorPalette {
