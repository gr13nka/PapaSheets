 ...
 *
 * **Экранный фильтр (Э7) сюда не доходит — и это решение, а не упущение.** В выгрузку всегда идёт
 * весь журнал: «выгрузил и недосчитался записей» — ошибка, которую замечают через недели и уже на
 * стороне заказчика, а «выгрузил лишнее» видно сразу при открытии файла. Цена этих двух исходов
 * несопоставима, поэтому [records] приходят сюда неотфильтрованными. Чтобы решение не выглядело
 * поломкой, при активном фильтре [ru.papasheets.ui.export.ExportDialog] предупреждает об этом прямо
