...
data class JournalQuery(
    ...
    /**
     * Даёт ли этот запрос ту же раскладку матрицы, что и следующий запуск приложения. Вид и фильтр
     * живут в памяти экрана и на запуске сбрасываются к значениям по умолчанию, а вот положение
     * вьюпорта [ru.papasheets.domain.LastPlace] хранит между запусками — и это положение осмысленно
     * ровно тогда, когда строки под ним останутся теми же. Сохранённое под фильтром, оно назавтра
     * указывало бы на другие дни. Порядок дат в матрице сюда не входит: она всегда по возрастанию.
     */
    val isDefaultMatrixLayout: Boolean
        get() = viewMode == ViewMode.MATRIX && filter.isEmpty
}
