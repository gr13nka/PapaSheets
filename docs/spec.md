...
Прораб ведёт на объекте журнал работ: дата + подрядчик + код локации + «вид работ» + до двух фото. Раньше — Google Sheets («Июнь.xlsx», матрица дата × 28 подрядчиков), но Sheets задыхается от встроенных фото и неудобен с телефона. PapaSheets — нативное Android-приложение, полностью офлайн, без сервера: таблица живёт в приложении, Google Sheets больше не нужен. Приложение развивается с сохранением данных установленных версий.
...
- **До двух фото на запись; текст без фото тоже допустим.** Источник: камера из приложения + галерея; оригинал остаётся в галерее, в приложение — сжатая копия (проверенный формат: вписать в 1280×720/720×1280, JPEG q≈0.72).
- Ключевой UX: быстрый рендер сотен строк с фото-превью; тап по фото → полноэкранный лайтбокс с пинч-зумом; **сильный пинч-зум-аут всей матрицы** («кто что сделал» одним взглядом); закреплённые шапки (дата + подрядчики). Больше ничего из Google Sheets не нужно.
...
- Экспорт: **xlsx со встроенными фото в существующем формате матрицы**, CSV, полный бэкап-файл. UI на русском и английском; язык следует системе либо выбирается в настройках приложения.
...
4. `GridModelBuilder(records, contractors, fields, valueColors) -> GridModel` — чистая функция раскладки «несколько строк на дату» (строк в дне = max записей одного подрядчика в этот день; заполнение сверху вниз по createdAt; дни всегда по возрастанию — порядок не настраивается).
...
## Историческая схема Room v1

Текущая схема — v8: группы и определения полей принадлежат `journalId`, значения лежат в
`record_values`, у записи два фото-слота. Несколько именованных таблиц могут иметь один месяц.
Миграция v7→v8 копирует прежнюю общую структуру в каждый журнал с детерминированными id;
записи, значения и оба фото сохраняются. Новая таблица получает группу и описание либо копию
структуры выбранной таблицы (включая пресеты, цвета и архив, но без записей и фото).

Ниже — исходная схема для контекста старых миграций:
...
`journals` → `journal/{id}` → `lightbox/{recordId}/{slot}`; `settings` (общеприложенческое) →
не вложен в журнал; `settings/{journalId}/table` → `settings/{journalId}/contractors` |
`settings/{journalId}/fields` (структура выбранной таблицы); `journal/{id}/list` — debug-маршрут без кнопки в UI, только по
URL. Форма записи — `ModalBottomSheet` внутри JournalScreen, не destination.

1. **JournalListScreen** — именованные таблицы с месяцем, FAB создания открывает выбор простой
   структуры или структуры предыдущей таблицы с живым предпросмотром; ⚙ в topBar открывает
   **SettingsScreen**.
2. **JournalScreen** — два вида одной выборки: MatrixView во весь экран (строки — дни, всегда по
   возрастанию) и плоский «Список» с сортировкой по любому столбцу в обе стороны (на матрицу не
   влияет); общий фильтр (подрядчики/группы, значения полей, поиск) применяется до раскладки;
   topBar: Назад, экспорт (иконка, открывает ExportDialog), ⋮ → **TableSettingsScreen**; круглый
   FAB «+», действия добавления колонки/группы; в матрице без фильтра видны все дни месяца,
   пустые дни не создают записей в БД. Тап по заголовку открывает редактор, тап по текстовой
   ячейке фокусирует соответствующее поле; тап по пустому слоту → форма с предзаполненными датой/подрядчиком; двойной тап зумит
   между «картиной месяца» и 1:1.
3. **RecordSheet** — дата (default сегодня), группа-подрядчик (dropdown, «+ Создать новую группу»
   заводит её не выходя из формы), поля из `field_defs` (в т.ч. встроенные «Локация» и «Вид
   работ», автодополнение по флагу поля), фото ×2 (камера/галерея), «Продолжить вчерашнее»
   (копирует подрядчика и значения полей, фото — новое). Можно начать с текста или фото; закрытие сохраняет непустой черновик.
4. **LightboxScreen** — fullscreen medium-фото с пинч-зумом, подпись, «Редактировать запись».
5. **SettingsScreen** («Настройки») — Язык; раздел «Данные» (сохранить бэкап; импорт бэкапа или
   xlsx-таблицы); раздел «Диагностика» (отправить логи).
6. **TableSettingsScreen** («Настройки таблицы») — имя таблицы, пояснение области действия и
   две строки: «Группы колонок» → **ContractorsScreen** (CRUD подрядчиков-групп, drag-reorder,
   цвет, архив); «Колонки» → **GroupSettingsScreen**.
7. **GroupSettingsScreen** — сверху фиксированный живой `MatrixView`-превью одной группы
   (`buildGroupPreview`, те же данные и раскраска, что у журнала), снизу список активных полей
   (drag-reorder, «+ Добавить поле», свёрнутый архив) либо редактор выбранного (название, подпись
   в шапке, ширина — слайдер на 4 пресета `ColumnWidth`, строк в ячейке, три переключателя,
   пресеты значений, архив/удаление); каждая правка пишется сразу, без «Сохранить».
8. **ExportDialog** — xlsx/CSV/бэкап + прогресс (xlsx на сотни фото — секунды, 50–75 МБ). Опция «без фото» была задумана как ответ на размер файла, но не пригодилась и убрана 2026-08-06.
