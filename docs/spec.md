...
1. **JournalListScreen** — карточки месяцев, FAB «Новый журнал», импорт бэкапа, настройки, выбор языка интерфейса.
2. **JournalScreen** — два вида одной выборки: MatrixView во весь экран и плоский «Список» с сортировкой по любому столбцу; общий фильтр (подрядчики, значения полей, поиск) применяется до раскладки; topBar: вид, фильтр, сортировка ↑↓, экспорт; круглый FAB «+»; тап по пустому слоту → форма с предзаполненными датой/подрядчиком.
