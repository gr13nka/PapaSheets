 ...
 *
 * Нижнюю границу задаёт не читаемость, а просмотрщик прораба: Google Sheets на телефоне вовсе не
 * рендерит встроенную картинку мельче порога — по замерам на устройстве (2026-07-22, лестница
 * размеров) фото с длинной стороной 28pt невидимо, 42pt видно. Отсюда 50 = 42 + [PHOTO_PADDING_PT]:
 * минимальная подтверждённо видимая величина. Компактные 36pt (фото 28pt, «разглядывают, увеличив
 * в Excel») делали фото невидимым в Sheets — это выглядело как «фото не выгрузились». Прежние 90pt
 * (M6) читались прямо в таблице, но убивали обзор. Уменьшать ниже 50 нельзя; замер делался на
 * портретных кадрах, порог по короткой стороне не проверялся — если пропадут альбомные фото,
 * поднимать до 64 (альбомная высота сравняется с проверенными 42pt).
 *
 ...
 */
internal const val PHOTO_ROW_HEIGHT_PT = 50.0
