 ...
 *
 * Нижнюю границу диктует просмотрщик прораба: Google Sheets на телефоне не рисует встроенную
 * картинку, которой тесно в её ячейке-якоре — ячейка выглядит пустой, хотя файл корректен и Excel
 * на компьютере рисует что угодно. Поэтому строка обязана быть выше фото на [PHOTO_PADDING_PT]:
 * 64 = 56 + 8, то есть 85.33px под фото в 74.67px, запас 10.67px при измеренном минимуме в 10px
 * (замеры 2026-08-06 — docs/evolution.md, «xlsx»). Ровно то же поле обязано быть и по горизонтали,
 * поэтому ширина колонки Ф считается от ЭТОЙ величины, а не от [PHOTO_BOX_PT] (см. `SheetXml`).
 *
 * «Компактные» 36pt (фото 28) делали фото невидимым — выглядело как «фото не выгрузились».
 * Прежние 90pt (M6) видимость давали, но растягивали журнал сильнее необходимого.
 *
