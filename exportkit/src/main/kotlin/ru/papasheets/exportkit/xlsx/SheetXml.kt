...
// под дату.
//
// Ширина колонки Ф считается от стороны фото-квадрата ([PHOTO_BOX_PT]), а не подбирается на глаз:
// картинка вставлена с явным размером и ничем не подрезается, поэтому колонка уже неё пустила бы
// фото поверх первой колонки поля — а на телефоне просмотрщик такое фото ужимает, и оно вовсе
// пропадает с экрана. Инвариант «фото не шире своей колонки» держит [Widths.ptToChars] и
// закрепляет тест `PhotoFitTest`; на глазок здесь ничего не сходится — до 2026-08-06 колонка была
// на 5px уже фото, и этого никто не видел.
