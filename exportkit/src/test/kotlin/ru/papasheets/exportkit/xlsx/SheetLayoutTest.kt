...
class SheetLayoutTest {
    ...
    fun `four fields widen the contractor group to six columns`() {
        ...
        // Ф — 10.72: ширина колонки считается от стороны фото-квадрата (PHOTO_BOX_PT), а не берётся
        // из эталона, иначе фото наезжало бы на первую колонку поля. Было 9.95 до 2026-08-06 —
        // Widths.ptToChars вычитала 5px, уже входящие в единицу ширины (см. PhotoFitTest).
        val widths = sheet.elements("col").map { it.getAttribute("width") }
        assertEquals(
            listOf(
                "10.5",
                "10.72", "10.72", "10.0", "11.0", "12.0", "13.0",
                "10.72", "10.72", "10.0", "11.0", "12.0", "13.0",
            ),
            widths,
        )
