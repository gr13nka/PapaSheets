...
class XlsxWriterTest {
    ...
    fun `xlsx with photos has correct merges, freeze, widths, header and inline strings`() {
        ...
        // Обе Ф считаются от высоты строки с фото (PHOTO_ROW_HEIGHT_PT = 64pt), а не задаются
        // отдельно: фото 56pt обязано лежать в колонке с тем же полем, что и в строке. 9.95 → 12.29
        // (2026-08-06) — 70px не хватало горизонтальным фото, замер на устройстве в PhotoFitTest.
        assertEquals("12.29", cols[1].getAttribute("width"))
        assertEquals("12.29", cols[2].getAttribute("width"))
